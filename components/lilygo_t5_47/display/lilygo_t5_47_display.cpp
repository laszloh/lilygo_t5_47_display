...
int LilygoT547Display::get_width_internal() { 
  ESP_LOGD(TAG, "Width: %d", EINK_WIDTH);
  return EINK_WIDTH;
}

int LilygoT547Display::get_height_internal() {
  ESP_LOGD(TAG, "Height: %d", EINK_HEIGHT);
  return EINK_HEIGHT;
}
...
void LilygoT547Display::setup() {
  ...
  if(fb)
    free(fb);
  fb = (uint8_t*)ps_malloc(EINK_BUFFER_SIZE);
  if(!fb) {
    ESP_LOGE(TAG, "Could not allocate buffer for display!");
    this->mark_failed();
    return;
  }
  memset(fb, 0, EINK_BUFFER_SIZE);
...
void LilygoT547Display::update() {
  ...
  ESP_LOGD(TAG, "Flush start");
  LilygoT547Display::flush_screen_changes();
  ESP_LOGD(TAG, "Flush end");
}
